...
class Flazz_Job_Engine {
    ...
    public function run_job( $job_id ) {
        error_log( '[Flazz AI] Job Engine: run_job started for ID ' . $job_id );
        // License Guard
        ...
        error_log( "[Flazz AI] Job Engine: Type=$job_type, Key=$keyword, URL=$rss_url, Max=$max_art" );
        ...
        if ( $job_type === 'keyword' ) {
            ...
            return "Tipe job tidak dikenal (ID: $job_id, Type: $job_type).";
        }
        ...
        if ( empty( $synthesis ) ) {
            ...
            foreach ( $articles as $article ) {
                ...
                if ( $this->is_link_processed( $article['link'] ) ) {
                    continue;
                }
            ...
            if ( empty( $source_contents ) ) return "Tidak ada artikel baru yang valid untuk diproses (Sudah diposting semua atau gagal ekstrak?).";
        ...
        if ( $show_img_source && ! empty( $final_image ) ) {
            $img_source_text = '';
            if ( $image_mode === 'generate_ai' ) {
                $img_source_text = 'Gambar dibuat oleh Flazz AI (Replicate Flux)';
            } elseif ( ! empty( $source_contents[0]['image'] ) && current( explode( '?', $final_image ) ) === current( explode( '?', $source_contents[0]['image'] ) ) ) {
                $source_name = ! empty( $source_contents[0]['sourceName'] ) ? $source_contents[0]['sourceName'] : 'Original Source';
                $img_source_text = esc_html( $source_name );
            } elseif ( $image_mode === 'pixabay' ) {
                $img_source_text = 'Pixabay (Bebas Royalti)';
            } else {
                $source_name = ! empty( $source_contents[0]['sourceName'] ) ? $source_contents[0]['sourceName'] : 'Web';
                $img_source_text = esc_html( $source_name );
            }
            
            if ( ! empty( $img_source_text ) ) {
                // Letakkan sumber foto di bagian paling atas konten (di bawah featured image)
                $content_with_links = '<p class="flazz-image-attribution" style="font-size: 11px; color: #888; font-style: italic; margin-top: 0; margin-bottom: 20px;">Sumber Foto: ' . $img_source_text . '</p>' . "\n" . $content_with_links;
            }
        }
        ...
        if ( $new_post_id ) {
            ...
            if ( ! empty( $final_image ) ) {
                ...
                if ( ! is_wp_error( $id ) ) {
                    ...
                    if ( $show_img_source && ! empty( $img_source_text ) ) {
                        // Taruh sebagai caption juga agar theme yang mendukung bisa menampilkan
                        wp_update_post( array(
                            'ID'           => $id,
                            'post_excerpt' => 'Sumber Foto: ' . $img_source_text
                        ) );
                    }
                }
            }
            ...
            try {
                ...
                $this->log( "run_job: Enrichment FAILED for post $new_post_id - " . $t->getMessage() );
            }
    ...
    public function enrich_post( $post_id, $title, $content, $force_auto_cat = false ) {
        ...
        try {
            ...
            $this->log( 'enrich_post: SEO Meta Error - ' . $t->getMessage() );
        }
        ...
        try {
            ...
            $this->log( 'enrich_post: Taxonomy Error - ' . $t->getMessage() );
        }
    }
