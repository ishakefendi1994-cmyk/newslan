...
export async function GET(
  ...
  unstable_noStore() // opt out of static caching so domain is always fresh
  const { sitemap } = await params
  const supabase = await createClient()
  const settings = await getSiteSettings()

  // Derive SITE_URL from the actual request host — always correct regardless of DB cache
  const host = request.headers.get('host') || ''
  const proto = request.headers.get('x-forwarded-proto') || 'https'
  const SITE_URL = host
    ? `${proto}://${host}`
    : (settings.site_url || process.env.NEXT_PUBLIC_SITE_URL || '')
...
<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">
  ${(games || []).map(g => `
  <url>
    <loc>${SITE_URL}/games/${g.slug}</loc>
    <lastmod>${new Date(g.updated_at || new Date()).toISOString()}</lastmod>
